...
units = options[index]["units"]
clients_data = options[index]["clients_data"]
    ...
    for key, value in sorted_combinations:
        ...
        if clients_data:
            ...
            client_column_values = pd.concat(
                [df[client_column_name] for df in client_data_frames]
            )
...
output_file = f"{sc.plots_directory}{addition}_usage"
